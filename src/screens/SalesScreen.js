// src/screens/SalesScreen.js — Version complète avec modal de création
...
export default function SalesScreen() {
  ...
  const [sales, setSales] = useState(MOCK_DATA.recentSales);
    ...
    <View style={{ flex: 1, backgroundColor: '#F5F5F5' }}>
      ...
      >
        ...
        <Card style={{ paddingVertical: 4 }}>
          ...
          {filtered.length === 0 && <Text style={styles.emptyText}>Aucune facture trouvée</Text>}
        </Card>

        <SectionTitle>Résumé du mois</SectionTitle>
        <Card>
          <RowBetween><Text style={styles.statLabel}>CA total</Text><Text style={[styles.statValue, { color: COLORS.primary }]}>{formatDA(4820000)}</Text></RowBetween>
          <Divider />
          <RowBetween><Text style={styles.statLabel}>Factures émises</Text><Text style={styles.statValue}>48</Text></RowBetween>
          <Divider />
          <RowBetween><Text style={styles.statLabel}>Payées</Text><Text style={[styles.statValue, { color: COLORS.success }]}>41</Text></RowBetween>
          <Divider />
          <RowBetween><Text style={styles.statLabel}>En attente</Text><Text style={[styles.statValue, { color: COLORS.warning }]}>7</Text></RowBetween>
          <Divider />
          <RowBetween style={{ marginBottom: 4 }}><Text style={styles.statLabel}>Taux recouvrement</Text><Text style={[styles.statValue, { color: COLORS.success }]}>87%</Text></RowBetween>
          <ProgressBar value={87} max={100} color={COLORS.success} />
        </Card>
            ...
            {selectedSale && (
              <>
                ...
                {[
                  ...
                  { label: 'Sous-total HT', value: formatDA(Math.round(selectedSale.total / 1.19)) },
                  { label: 'TVA (19%)', value: formatDA(selectedSale.total - Math.round(selectedSale.total / 1.19)) },
                  { label: 'Total TTC', value: formatDA(selectedSale.total), color: COLORS.primary, bold: true },
                ].map((row, i) => (
      ...
      <NewSaleModal visible={newSaleVisible} onClose={() => setNewSaleVisible(false)} onSaved={onRefresh} />
    </View>
  );
}
