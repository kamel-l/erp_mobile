...
const KEYS = {
  ...
  PENDING_VENTES: 'erp_pending_ventes',
  PENDING_CLIENTS: 'erp_pending_clients',
  PENDING_IMAGES: 'erp_pending_images',
};
...
class SyncService {
  ...
  async _detectBestUrl() {
    ...
    // 1. Essayer d'abord le WiFi local
  ...
  async _ping(baseUrl) {
    try {
      // Correction : utiliser /api/ping au lieu de /api/health
  ...
  // ───────────────────────────────────────────────────────────
  //  Requêtes HTTP avec gestion de token
  // ───────────────────────────────────────────────────────────
  ...
  async fullSync(options = {}) {
    ...
    try {
      const lastSync = sinceLastSync ? await AsyncStorage.getItem(KEYS.LAST_SYNC) : null;
  ...
  // ───────────────────────────────────────────────────────────
  //  PUSH : envoyer les données en attente
  // ───────────────────────────────────────────────────────────
  async _pushPending() {
    ...
    try {
      ...
      console.warn('⚠️ Push échoué, données conservées :', e.message);
  ...
  async createVente(data) {
    ...
    data.sale_date = data.sale_date || new Date().toISOString().replace('T', ' ').slice(0, 19);
  ...
  async registerPushToken(token, deviceName = 'mobile') {
    await this._detectBestUrl();
    if (!this.isOnline) return false;
    try {
      await this._post('/api/mobile/register-token', { token, device_name: deviceName });
      return true;
    } catch (e) {
      console.error(e);
      return false;
